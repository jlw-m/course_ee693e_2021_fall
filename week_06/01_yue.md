...
***
## Paper Summary
This paper presents the first RF-based respiration monitoring system called “DeepBreath” that can recover the breathing signals of multiple individuals even when they are located by zero distance. Respiration monitoring during sleep can be useful to diagnose sleep disorders, sleep disturbed breathing (SDB), cardiac arrest, sleep apnea etc. RF signals have been used to track a person’s respiration allowing to track the sleep quality and stages of a subject and monitor sleep apnea and other sleep disordered breathing without any body contact.  The design of DeepBreath combines three components: (a) Breathing  Separation, (b) Motion Detection and (c) Identity Matching. A breathing separation module is being used in DeepBreath to reconstruct the correct breathing signals of multiple co-located individuals. The reliability of the motion detection is ensured thorugh a convolutional neural network trained to identify movements of the monitored persons. The paper repoted an average precision of 0.933 for motion detection. The approach of extracting Multi-Person Respiration from entangled RF signal is not very easy to integrated into people’s daily lives because the system might get affected when other people are present in the vicinity of the subject (which is almost all the time). In this paper, a solution is presented where the multiple RF signal interference is modeled, and independent component analysis (ICA) is used to recover the original breathing of individuals. The proposed method can attribute each signal to the source subject with very high accuracy and precision, and the distance and position of the subjects does not affect the data quality.
